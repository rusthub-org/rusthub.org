...
pub async fn file_copy(
    ...
    let file_write = file.write_all(&field_chunk.unwrap()).await;
...
impl<T: Read + Unpin> Stream for BufferedBytesStream<T> {
    ...
    fn poll_next(
        ...
        let mut buf = [0u8; 10240]; // [0u8; 2048]

        let pointer: Pin<&mut T> = Pin::new(&mut self.inner);
        match futures::ready!(pointer.poll_read(cx, &mut buf)) {
